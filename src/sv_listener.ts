...
export class sv_listener implements svl.SystemVerilogParserListener {
  ...
  getIncompleteIdent(node: pgtree.TerminalNode | pgtree.ErrorNode) {
    ...
    for (let i = node.symbol.tokenIndex; i >= 0; --i) {
      ...
      let pre_symbol = svx.SystemVerilogLexer.VOCABULARY.getSymbolicName(
        pre_token.type,
      );
  ...
  findHintForCompletion(node: pgtree.TerminalNode | pgtree.ErrorNode) {
    ...
    let name = svx.SystemVerilogLexer.VOCABULARY.getDisplayName(
      node.symbol.type,
    );
    let lit = svx.SystemVerilogLexer.VOCABULARY.getLiteralName(
      node.symbol.type,
    );
    let sym = svx.SystemVerilogLexer.VOCABULARY.getSymbolicName(
      node.symbol.type,
    );
    ...
    this.cursor_hint.pos = new position(
      node.symbol.line,
  ...
  //
  // ==========================================================================
  exitList_of_net_decl_assignments(
    ctx: svp.List_of_net_decl_assignmentsContext,
  ) {
    ctx['rets'] = [];
  ...
  //
  // ==========================================================================
  exitList_of_variable_decl_assignments(
    ctx: svp.List_of_variable_decl_assignmentsContext,
  ) {
    ctx['rets'] = [];
  ...
  //
  // ==========================================================================
  exitFunction_data_type_or_implicit(
    ctx: svp.Function_data_type_or_implicitContext,
  ) {
  ...
  enterFunction_body_declaration_0(
    ctx: svp.Function_body_declaration_0Context,
  ) {}
  enterFunction_body_declaration_1(
    ctx: svp.Function_body_declaration_1Context,
  ) {
    let idents = colrets(ctx.ident(), '');
    let cscope = colrets(ctx.class_scope(), '');
  ...
  exitList_of_parameter_assignments(
    ctx: svp.List_of_parameter_assignmentsContext,
  ) {
    if (!this.cursor_belong) {
      return;
    }
  ...
  exitTime_literal(ctx: svp.Time_literalContext) {
    ctx['rets'] = new sv_timeunit(
      ctx.UNSIGNED_NUMBER().text,
      ctx.TIME_UNIT().text,
    );
  }
  ...
  exitTimeunits_declaration_0(ctx: svp.Timeunits_declaration_0Context) {
    ...
    db.timeunit.unit = ctx.time_literal(1)
      ? ctx.time_literal(1)['rets']
      : undefined;
  }
  ...
  exitData_type_or_implicit(ctx: svp.Data_type_or_implicitContext) {
    ...
    ctx['rets'] = objmerge(
      ctx['rets'],
      colrets(ctx.implicit_data_type(), 'rets')[0],
    );
    ...
    if (this.search_ident === ident.text) {
      this.ident_locs.push({
        start: {
          ...
          character: ident.symbol.charPositionInLine,
        },
        end: {
          line: ident.symbol.line,
          character: ident.symbol.charPositionInLine + ident.symbol.text.length,
        },
      });
    }
  }
